...
class XenManager:
    ...
    def get_max_term_days(self):
        return int(self._xen_contract.functions.getCurrentMaxTerm().call() / 3600 / 24)
    ...
    def cal_estimated_xen(term, global_rank, rank, amp, eaa):
        return term * math.log2(max(global_rank - rank, 2)) * amp * (1 + eaa / 1000)
    ...
    def handle_each_wallet(self, each_wallet, info_datas, global_rank, row_no):
        ...
        if rank == 0:
            print(f'Wallet {each_wallet[No_Index]} {addr} has no Xen mint info, ignored.')
            return
        xen_count = self.cal_estimated_xen(term, global_rank, rank, amp, eaa)
        mint_info_data = [each_wallet[No_Index], addr, term, self.local_time(maturityTs), int(xen_count)]
    ...
    def transfer_money(self, amount, from_wallet, csv_path):
        ...
        for to_wallet in to_wallets:
            ...
            print(params)
            print(
                f'Transferring {amount} from {from_wallet[Addr_Index]} to {to_wallet[Addr_Index]}, Chain:{self._chain_id}, Nonce:{params["nonce"]}, GasLimit:{params["gas"]}')
